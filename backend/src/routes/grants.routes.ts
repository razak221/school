...
router.post('/mdm-log', verifyToken, requireRole(['admin', 'teacher']), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const log = await MidDayMeal.findOneAndUpdate(
      ...
      {
        menuServed: menuServed || 'Fresh Steamed Rice & Dal',
        riceConsumedKg: Number(riceConsumedKg) || 0,
        vegetablesExpense: Number(vegetablesExpense) || 0,
        studentsServedCount: Number(studentsServedCount) || 0,
        totalEnrolledCount: Number(totalEnrolledCount) || Number(studentsServedCount) || 0,
