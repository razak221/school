...
RALPH_DEFAULT_AGENT="claude" # set by install CLI; overridden by --agent
...
# Print the complete captured log below a formatted fatal error message.
print_full_error_log() {
  local log_text="$1"
